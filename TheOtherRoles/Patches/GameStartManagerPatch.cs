            ...
            public static void Postfix(GameStartManager __instance) {
                ...
                if (AmongUsClient.Instance.AmHost) {
                    ...
                    foreach (InnerNet.ClientData client in AmongUsClient.Instance.allClients.ToArray()) {
                        ...
                        if (dummyComponent != null && dummyComponent.enabled)
                            ...
                            blockStart = true;
                            message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} は違うバージョンの The Other Rolesを導入しているか、The Other Rolesが導入されていません\n</color>";
                        } else {
                            PlayerVersion PV = playerVersions[client.Id];
                            int diff = TheOtherRolesPlugin.Version.CompareTo(PV.version);
                            if (diff > 0) {
                                message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} は、古いバージョンのThe Other Rolesを入れています (v{playerVersions[client.Id].version.ToString()})\n</color>";
                                blockStart = true;
                            } else if (diff < 0) {
                                message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} は、新しいバージョンのThe Other Rolesを入れています (v{playerVersions[client.Id].version.ToString()})\n</color>";
                                blockStart = true;
                ...
                if (!AmongUsClient.Instance.AmHost) {
                    if (!playerVersions.ContainsKey(AmongUsClient.Instance.HostId) || TheOtherRolesPlugin.Version.CompareTo(playerVersions[AmongUsClient.Instance.HostId].version) != 0) {
                        ...
                        __instance.GameStartText.text = $"<color=#FF0000FF>ホストがThe Other Rolesを入れてないか、バージョンが違います。\n{Math.Round(10 - kickingTimer)}秒後にキックされます。</color>";
                        __instance.GameStartText.transform.localPosition = __instance.StartButton.transform.localPosition + Vector3.up * 2;
                    } else {
                        __instance.GameStartText.transform.localPosition = __instance.StartButton.transform.localPosition;
