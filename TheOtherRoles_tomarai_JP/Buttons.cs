    ...
    static class HudManagerStartPatch
    {
        ...
        public static void Postfix(HudManager __instance)
        {
            ...
            deputyHandcuffButton = new CustomButton(
                ...
                () => {
                    if (deputyButtonHandcuffsText != null) deputyButtonHandcuffsText.text = $"残り{Deputy.remainingHandcuffs}個";
            ...
            hackerVitalsButton = new CustomButton(
               ...
               () => {
                   ...
                   hackerVitalsButton.actionButton.OverrideText(PlayerControl.GameOptions.MapId == 1 ? "ドアログ" : "バイタル");
               ...
               false,
               PlayerControl.GameOptions.MapId == 1 ? "ドアログ" : "バイタル"
           );
            ...
            securityGuardCamButton = new CustomButton(
                ...
                () => {
                    ...
                    securityGuardCamButton.actionButton.OverrideText(PlayerControl.GameOptions.MapId == 1 ? "ドアログ" : "カメラ設置");
                ...
                false,
                PlayerControl.GameOptions.MapId == 1 ? "ドアログ" : "カメラ設置"
            );
            ...
            mediumButton = new CustomButton(
                ...
                () => {
                    ...
                    int randomNumber = Medium.target.killerIfExisting?.PlayerId == Mini.mini?.PlayerId ? TheOtherRoles_tomarai_JP.rnd.Next(3) : TheOtherRoles_tomarai_JP.rnd.Next(4);
                    string typeOfColor = Helpers.isLighterColor(Medium.target.killerIfExisting.Data.DefaultOutfit.ColorId) ? "明るい" : "暗い";
                    float timeSinceDeath = ((float)(Medium.meetingStartTime - Medium.target.timeOfDeath).TotalMilliseconds);
                    string name = " (" + Medium.target.player.Data.PlayerName + ")";


                    if (randomNumber == 0) msg = "あなたの役職は? 私の役職は " + RoleInfo.GetRolesString(Medium.target.player, false) + "です。" + name;
                    else if (randomNumber == 1) msg = "あなたを殺した人の色は? 私を殺したのは " + typeOfColor + " 色です。" + name;
                    else if (randomNumber == 2) msg = "いつ死んだの? 私は会議が始まる " + Math.Round(timeSinceDeath / 1000) + "秒前に死にました。" + name;
                    else msg = "あなたを殺した人の役職は? 私を殺したのは " + RoleInfo.GetRolesString(Medium.target.killerIfExisting, true) + "です。" + name;
            ...
            mayorMeetingButton = new CustomButton(
               ...
               () => {
                   mayorMeetingButton.actionButton.OverrideText("会議を開く");
                   bool sabotageActive = false;
                   foreach (PlayerTask task in PlayerControl.LocalPlayer.myTasks)
                       if (task.TaskType == TaskTypes.FixLights || task.TaskType == TaskTypes.RestoreOxy || task.TaskType == TaskTypes.ResetReactor || task.TaskType == TaskTypes.ResetSeismic || task.TaskType == TaskTypes.FixComms || task.TaskType == TaskTypes.StopCharles
                           || SubmergedCompatibility.isSubmerged() && task.TaskType == SubmergedCompatibility.RetrieveOxygenMask)
